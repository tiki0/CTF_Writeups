# Puzzle - Securinets CTF 2025 Writeup

## Challenge Information
- **Challenge Name:** Puzzle
- **Category:** Web
- **CTF:** 2025 Securinets Qualifier
...
The challenge had the user find several misconfigurations in a web application that chained together would grant the user admin privileges.

With those privileges the user could download a password protected `secrets.zip` file along with a `dbconnect.exe` file.
...
This will be quite useful later. Now I need to find the uuid of a privileged user. Inspecting the `models.py` file we see this piece of code:
...
Of course the password supplied is just a dummy, but what is useful here is that there's a privileged user named admin.
...
So, my exploitation chain was:
1. Sending a collaboration request to admin
2. Checking the HTML, taking the uuid
3. Sending a request to `/collab/accept/[request uuid]`
4. Finally, looking at the accepted request to find the admin's uuid

Now, armed with admin's uuid, I send a request to `/users/[admin uuid]` and get admin's login.
...
`secrets.zip` is password protected, I first try admin's login but it fails, I then go onto inspecting the `dbconnect.exe` file by using `strings dbconnect.exe` and see the following line:
...
I was able to crack admin's password but it proved to be useless, unless I missed something?
